...
<p>To set up the initial routing, we pass a <code>Routes</code> function describing how to handle routing, to <a href="https://docs.rs/seed/0.2.5/seed/struct.App.html#method.build">App::build</a><code>'s  routes</code> method.</p>
...
<a class="sourceLine" id="cb3-12" title="12">            <span class="pp">seed::</span>push_path(<span class="pp">vec!</span><span class="op">[</span>page<span class="op">]</span>);</a>
<a class="sourceLine" id="cb3-13" title="13">            update(<span class="pp">Msg::</span>ChangePage(page), model)</a>
<a class="sourceLine" id="cb3-14" title="14">        <span class="op">}</span>,</a>
<a class="sourceLine" id="cb3-15" title="15">        <span class="pp">Msg::</span>RouteGuidePage(guide_page) =&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb3-16" title="16">            <span class="pp">seed::</span>push_path(<span class="pp">vec!</span><span class="op">[</span><span class="st">&quot;guide&quot;</span>, guide_page<span class="op">]</span>);</a>
<a class="sourceLine" id="cb3-17" title="17">            update(<span class="pp">Msg::</span>ChangeGuidePage(guide_page), model)</a>
<a class="sourceLine" id="cb3-18" title="18">        <span class="op">}</span>,</a>
<a class="sourceLine" id="cb3-19" title="19">        <span class="co">// This is separate, because nagivating the route triggers state updates, which would</span></a>
<a class="sourceLine" id="cb3-20" title="20">        <span class="co">// trigger an additional push state.</span></a>
<a class="sourceLine" id="cb3-21" title="21">        <span class="pp">Msg::</span>ChangePage(page) =&gt; Render(Model <span class="op">{</span>page, ..model<span class="op">}</span>),</a>
<a class="sourceLine" id="cb3-22" title="22">        <span class="pp">Msg::</span>ChangeGuidePage(guide_page) =&gt; Render(Model <span class="op">{</span>guide_page, page: <span class="pp">Page::</span>Guide, ..model<span class="op">}</span>),</a>
...
<p>Notice how the <code>Route</code> messages above call <a href="https://docs.rs/seed/0.2.5/seed/routing/fn.push_path.html">seed::push_path</a>, and the <code>Change</code> messages are called in the <code>routes</code> function, and are recursively called in the update function. <code>push_path</code> is a convenience function for <a href="https://docs.rs/seed/0.2.5/seed/routing/fn.push_route.html">seed::push_route</a>. <code>push_route</code> accepts a single parameter: a <code>Url</code> struct, which you can create with <code>seed::Url::new</code> . It accepts the path as a <code>Vec</code> of items that implement <code>ToString</code> (eg <code>String</code>, <code>&amp;str</code>, numbers), and makes the rest of the fields <code>None</code>. If you wish to define one of these fields, there are additional methods you can chain together, eg:</p>
...
<a class="sourceLine" id="cb4-4" title="4">        .search(<span class="st">&quot;textaftersearch&quot;</span>)</a>
<a class="sourceLine" id="cb4-5" title="5">)</a></code></pre></div>
<p><code>push_path</code> doesn't support <code>hash</code>, <code>search</code>, or <code>title</code>, but has cleaner syntax; it takes the path, as described above, as its only parameter. When a page is loaded or browser naviation occurs (eg back button), Seed uses the <code>routes</code> func you provided to determine which message to call.</p>
<p>Notice how we keep ChangePage and RoutePage separate in our example. Do not call <code>push_path</code> or <code>push_route</code> from one of these messages, or you'll end up with recusions/unwanted behavior: <code>ChangePage</code> in our example performs the action associated with routing, while <code>RoutePage</code> updates our route history, then recursively calls <code>ChangePage</code>. If you were to attempt this in the same message, each browser navigation event would add a redundant route history entry, interfering with navigation. `</p>
<p>We call routing messages from in-app navigation events, like this:</p>
<div class="sourceCode" id="cb5"><pre class="sourceCode rust"><code class="sourceCode rust"><a class="sourceLine" id="cb5-1" title="1"><span class="pp">h2!</span><span class="op">[</span> simple_ev(<span class="pp">Ev::</span>Click, <span class="pp">Msg::</span>RoutePage), <span class="st">&quot;Guide&quot;</span> <span class="op">]</span></a></code></pre></div>
<p>Or programatically using lifecycle hooks:</p>
<div class="sourceCode" id="cb6"><pre class="sourceCode rust"><code class="sourceCode rust"><a class="sourceLine" id="cb6-1" title="1">    did_mount(<span class="kw">move</span> |_| <span class="op">{</span></a>
<a class="sourceLine" id="cb6-2" title="2">        <span class="kw">if</span> model.logged_in <span class="op">{</span></a>
<a class="sourceLine" id="cb6-3" title="3">            state.update(<span class="pp">Msg::</span>RoutePage(<span class="pp">Page::</span>Home))</a>
