...
<p>To set up the initial routing, we pass a <code>routes</code> function describing how to handle routing, to <a href="https://docs.rs/seed/0.2.9/seed/struct.App.html#method.build">App::build</a>'s <code>routes</code> method.</p>
...
<p>Your <code>routes</code> function outputs the message that handles the routing, and accepts a ref to a <a href="https://docs.rs/seed/0.2.9/seed/routing/struct.Url.html">Url struct</a> describing the route, which routes has the following fields:</p>
...
<p>Notice how the <code>Route</code> messages above call <a href="https://docs.rs/seed/0.2.9/seed/routing/fn.push_path.html">seed::push_path</a>, and the <code>Change</code> messages are called in the <code>routes</code> function, and are recursively called in the update function. <code>push_path</code> is a convenience function for <a href="https://docs.rs/seed/0.2.9/seed/routing/fn.push_route.html">seed::push_route</a>. <code>push_route</code> accepts a single parameter: a <code>Url</code> struct, which you can create with <a href="https://docs.rs/seed/0.2.9/seed/routing/struct.Url.html#method.new">seed::Url::new</a> . It accepts the path as a <code>Vec</code> of items that implement <code>ToString</code> (eg <code>String</code>, <code>&amp;str</code>, numbers), and makes the rest of the fields <code>None</code>. If you wish to define one of these fields, there are additional methods you can chain together, eg:</p>
