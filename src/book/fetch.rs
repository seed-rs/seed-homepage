...
<p>We use the <a href="https://docs.rs/seed/0.3.6/seed/fetch/struct.Request.html">seed::Request</a> struct to make HTTP requests in the browser, wrapping the <a href="https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API">Fetch API</a>. To use this, we need to include <code>futures = "^0.1.26"</code> in <code>Cargo.toml</code>. The <a href="https://docs.rs/seed/0.3.6/seed/fetch/index.html">Fetch module</a> is standalone: It can be used with any wasm-bindgen program.</p>
<p>Example, where we update the state on initial load (similar to the <code>server_interaction</code> example in the repo) from a server. It demonstrates a <code>GET</code> request, and deserializing JSON data.</p>
...
<a class="sourceLine" id="cb1-17" title="17">    FetchData,</a>
<a class="sourceLine" id="cb1-18" title="18">    DataFetched(<span class="pp">fetch::</span>FetchObject&lt;Branch&gt;),</a>
<a class="sourceLine" id="cb1-19" title="19">    OnFetchError <span class="op">{</span></a>
<a class="sourceLine" id="cb1-20" title="20">        label: &amp;<span class="ot">&#39;static</span> <span class="dt">str</span>,</a>
<a class="sourceLine" id="cb1-21" title="21">        fail_reason: <span class="pp">fetch::</span>FailReason,</a>
...
<a class="sourceLine" id="cb1-25" title="25"><span class="kw">fn</span> fetch_data() -&gt; <span class="kw">impl</span> Future&lt;Item = Msg, Error = Msg&gt; <span class="op">{</span></a>
...
<a class="sourceLine" id="cb1-34" title="34">                .skip()</a>
<a class="sourceLine" id="cb1-35" title="35">                .perform_cmd(fetch_data());</a>
<a class="sourceLine" id="cb1-36" title="36">        <span class="op">}</span></a>
<a class="sourceLine" id="cb1-37" title="37"></a>
<a class="sourceLine" id="cb1-38" title="38">        <span class="pp">Msg::</span>DataFetched(fetch_object) =&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb1-39" title="39">            <span class="kw">match</span> fetch_object.response() <span class="op">{</span></a>
<a class="sourceLine" id="cb1-40" title="40">                <span class="cn">Ok</span>(response) =&gt; model.branch = response.data,</a>
<a class="sourceLine" id="cb1-41" title="41">                <span class="cn">Err</span>(fail_reason) =&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb1-42" title="42">                    orders</a>
<a class="sourceLine" id="cb1-43" title="43">                        .send_msg(<span class="pp">Msg::</span>OnFetchError <span class="op">{</span></a>
<a class="sourceLine" id="cb1-44" title="44">                            label: <span class="st">&quot;Fetching repository info failed&quot;</span>,</a>
<a class="sourceLine" id="cb1-45" title="45">                            fail_reason,</a>
<a class="sourceLine" id="cb1-46" title="46">                        <span class="op">}</span>)</a>
<a class="sourceLine" id="cb1-47" title="47">                        .skip();</a>
...
<a class="sourceLine" id="cb1-52" title="52">        <span class="pp">Msg::</span>OnFetchError <span class="op">{</span> label, fail_reason <span class="op">}</span> =&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb1-53" title="53">            <span class="pp">error!</span>(<span class="pp">format!</span>(<span class="st">&quot;Fetch error - {} - {:#?}&quot;</span>, label, fail_reason));</a>
<a class="sourceLine" id="cb1-54" title="54">            orders.skip();</a>
...
<a class="sourceLine" id="cb1-59" title="59"><span class="kw">fn</span> view(model: &amp;Model) -&gt; El&lt;Msg&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb1-60" title="60">    <span class="pp">div!</span><span class="op">[</span><span class="pp">format!</span>(</a>
<a class="sourceLine" id="cb1-61" title="61">        <span class="st">&quot;Repo info: Name: {}, SHA: {}&quot;</span>,</a>
<a class="sourceLine" id="cb1-62" title="62">        model.branch.name, model.branch.commit.sha</a>
...
<a class="sourceLine" id="cb1-67" title="67"><span class="at">#[</span>wasm_bindgen<span class="at">]</span></a>
<a class="sourceLine" id="cb1-68" title="68"><span class="kw">pub</span> <span class="kw">fn</span> render() <span class="op">{</span></a>
<a class="sourceLine" id="cb1-69" title="69">    <span class="kw">let</span> app = <span class="pp">seed::App::</span>build(<span class="pp">Model::</span><span class="kw">default</span>(), update, view)</a>
<a class="sourceLine" id="cb1-70" title="70">        .finish()</a>
<a class="sourceLine" id="cb1-71" title="71">        .run();</a>
...
<div class="sourceCode" id="cb2"><pre class="sourceCode rust"><code class="sourceCode rust"><a class="sourceLine" id="cb2-1" title="1"><span class="kw">fn</span> view(model: &amp;Model) -&gt; <span class="dt">Vec</span>&lt;El&lt;Msg&gt;&gt; <span class="op">{</span></a>
...
<a class="sourceLine" id="cb3-16" title="16">    MessageSent(<span class="pp">fetch::</span>FetchObject&lt;ResponseBody&gt;),</a>
<a class="sourceLine" id="cb3-17" title="17">    OnFetchError <span class="op">{</span></a>
<a class="sourceLine" id="cb3-18" title="18">        label: &amp;<span class="ot">&#39;static</span> <span class="dt">str</span>,</a>
<a class="sourceLine" id="cb3-19" title="19">        fail_reason: <span class="pp">fetch::</span>FailReason,</a>
...
<a class="sourceLine" id="cb3-29" title="29">        <span class="pp">Msg::</span>MessageSent(fetch_object) =&gt; <span class="kw">match</span> fetch_object.response() <span class="op">{</span></a>
<a class="sourceLine" id="cb3-30" title="30">            <span class="cn">Ok</span>(response) =&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb3-31" title="31">                <span class="pp">log!</span>(<span class="pp">format!</span>(<span class="st">&quot;Response data: {:#?}&quot;</span>, response.data));</a>
<a class="sourceLine" id="cb3-32" title="32">                orders.skip();</a>
<a class="sourceLine" id="cb3-33" title="33">            <span class="op">}</span></a>
<a class="sourceLine" id="cb3-34" title="34">            <span class="cn">Err</span>(fail_reason) =&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb3-35" title="35">                orders</a>
<a class="sourceLine" id="cb3-36" title="36">                    .send_msg(<span class="pp">Msg::</span>OnFetchError <span class="op">{</span></a>
<a class="sourceLine" id="cb3-37" title="37">                        label: <span class="st">&quot;Sending message failed&quot;</span>,</a>
<a class="sourceLine" id="cb3-38" title="38">                        fail_reason,</a>
<a class="sourceLine" id="cb3-39" title="39">                    <span class="op">}</span>)</a>
<a class="sourceLine" id="cb3-40" title="40">                    .skip();</a>
...
<a class="sourceLine" id="cb3-44" title="44">        <span class="pp">Msg::</span>OnFetchError <span class="op">{</span> label, fail_reason <span class="op">}</span> =&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb3-45" title="45">            <span class="pp">log!</span>(<span class="pp">format!</span>(<span class="st">&quot;Fetch error - {} - {:#?}&quot;</span>, label, fail_reason));</a>
<a class="sourceLine" id="cb3-46" title="46">            orders.skip();</a>
...
<a class="sourceLine" id="cb3-51" title="51"><span class="kw">fn</span> send_message() -&gt; <span class="kw">impl</span> Future&lt;Item = Msg, Error = Msg&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb3-52" title="52">    <span class="kw">let</span> message = RequestBody <span class="op">{</span></a>
<a class="sourceLine" id="cb3-53" title="53">        name: <span class="st">&quot;Mark Watney&quot;</span>.into(),</a>
<a class="sourceLine" id="cb3-54" title="54">        email: <span class="st">&quot;mark@crypt.kk&quot;</span>.into(),</a>
<a class="sourceLine" id="cb3-55" title="55">        message: <span class="st">&quot;I wanna be like Iron Man&quot;</span>.into(),</a>
<a class="sourceLine" id="cb3-56" title="56">    <span class="op">}</span>;</a>
<a class="sourceLine" id="cb3-57" title="57"></a>
<a class="sourceLine" id="cb3-58" title="58">    <span class="pp">Request::</span>new(CONTACT_URL.into())</a>
<a class="sourceLine" id="cb3-59" title="59">        .method(<span class="pp">Method::</span>Post)</a>
<a class="sourceLine" id="cb3-60" title="60">        .header(<span class="st">&quot;Content-Type&quot;</span>, <span class="st">&quot;application/json&quot;</span>)</a>
<a class="sourceLine" id="cb3-61" title="61">        .send_json(&amp;message)</a>
<a class="sourceLine" id="cb3-62" title="62">        .fetch_json(<span class="pp">Msg::</span>MessageSent)</a>
<a class="sourceLine" id="cb3-63" title="63"><span class="op">}</span></a>
<a class="sourceLine" id="cb3-64" title="64"></a>
<a class="sourceLine" id="cb3-65" title="65"><span class="kw">fn</span> view(model: &amp;Model) -&gt;El&lt;Msg&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb3-66" title="66">    <span class="pp">button!</span><span class="op">[</span></a>
<a class="sourceLine" id="cb3-67" title="67">        simple_ev(<span class="pp">Ev::</span>Click, <span class="pp">Msg::</span>SendMessage),</a>
<a class="sourceLine" id="cb3-68" title="68">        <span class="st">&quot;Send an urgent message (see console log)&quot;</span></a>
