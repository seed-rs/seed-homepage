...
<p>The update <a href="https://doc.rust-lang.org/book/ch03-03-how-functions-work.html">function</a> you pass to <code>seed::App::build(</code> describes how the state should change, upon receiving each type of message. It's the only place where the model is changed. It accepts a message, and model as parameters, and returns an <code>Update</code> enum: This, and its variants are imported in the prelude, and has variants of <code>Render</code> and <code>Skip</code>. Each wrap a model. Render triggers a rendering update, and will be used in most cases. <code>Skip</code> updates the model without triggering a render, and is useful in animations. Note that it doesn't update the model in place: It returns a new one.</p>
<p>Example:</p>
...
<a class="sourceLine" id="cb7-4" title="4">        <span class="pp">Msg::</span>B =&gt; Render(do_other_things(do_things(model))),</a>
...
<p>Here's a recursive equivalent. Currently verbose due to not returning the model directly.</p>
...
<a class="sourceLine" id="cb8-4" title="4">        <span class="pp">Msg::</span>B =&gt; <span class="op">{</span></a>
<a class="sourceLine" id="cb8-5" title="5">            <span class="kw">match</span> update(<span class="pp">Msg::</span>A, model) <span class="op">{</span></a>
<a class="sourceLine" id="cb8-6" title="6">                Render(m) =&gt; Render(do_other_things(m))</a>
<a class="sourceLine" id="cb8-7" title="7">                Skip(m) =&gt; Render(do_other_things(m))</a>
...
<p>These macros accept any combination of the following parameters: - One <a href="https://docs.rs/seed/0.2.4/seed/dom_types/struct.Attrs.html">Attrs</a> struct - One <a href="https://docs.rs/seed/0.2.4/seed/dom_types/struct.Style.html">Style</a> struct - One or more <a href="https://docs.rs/seed/0.2.4/seed/dom_types/struct.Listener.html">Listener</a> structs, which handle events - One or more <code>Vec</code>s of <code>Listener</code> structs - One <code>String</code> or <code>&amp;str</code> representing a node text - One or more <a href="https://docs.rs/seed/0.2.4/seed/dom_types/struct.El.html">El</a> structs, representing a child - One or more Vecs of <code>El</code> structs, representing multiple children</p>
...
<p>Views are described using <a href="https://docs.rs/seed/0.2.4/seed/dom_types/struct.El.html">El</a> structs, defined in the <a href="https://docs.rs/seed/0.2.4/seed/dom_types/index.html">seed::dom_types</a> module.</p>
...
<a class="sourceLine" id="cb11-4" title="4">    <span class="pp">div!</span><span class="op">[</span> <span class="pp">attrs!</span><span class="op">{</span><span class="pp">At::</span>Class =&gt; <span class="st">&quot;hardly-any&quot;</span><span class="op">}</span>, </a>
...
<a class="sourceLine" id="cb12-3" title="3">    attributes.add_multiple(<span class="pp">At::</span>Class, <span class="pp">vec!</span><span class="op">[</span><span class="st">&quot;A-modicum-of&quot;</span>, <span class="st">&quot;hardly-any&quot;</span><span class="op">]</span>);</a>
