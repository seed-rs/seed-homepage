...
<p>You may log things to the JS console using the following 3 functions: <code>seed::log</code>, <code>seed::error</code>, and <code>seed::debug</code>. The first two accept a single argument which implements <code>ToString</code>, eg <code>String</code>, <code>&amp;str</code> etc. The third accepts one that implements <code>Debug</code>. Each has a corresponding macro: <code>log!</code>, <code>error!</code>, and <code>debug!</code>, which work in a similar way, but accept multiple arguments, which will be displayed separated by spaces.</p>
