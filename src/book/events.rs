...
<p>We handle events triggered by the overall window specially, since it doesn't fit directly into our virtual DOM. We pass to <code>Seed::App::build::window_events()</code> a function that accepts a ref to <code>Model</code>, and returns a <code>Vec&lt;dom_types::Listener&gt;</code>. We use it to control which listeners are attached to the window based on the model. Excerpt from the <a href="https://github.com/David-OConnor/seed/blob/master/examples/window_events/src/lib.rs">window_events</a> example:</p>
...
<a class="sourceLine" id="cb10-18" title="18"><span class="kw">fn</span> window_events(model: &amp;Model) -&gt; <span class="dt">Vec</span>&lt;<span class="pp">seed::dom_types::</span>Listener&lt;Msg&gt;&gt; <span class="op">{</span></a>
