...
<p>The analog of components in frameworks like React are normal Rust functions that that return <a href="https://docs.rs/seed/0.3.1/seed/dom_types/struct.El.html">El</a>s. These functions take parameters that are not treated in a way equivalent to attributes on native DOM elements; they just provide a way to organize your code. In practice, they're used in a way similar to components in React.</p>
